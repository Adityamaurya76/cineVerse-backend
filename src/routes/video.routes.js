...
router.route("/create").post(verifyJWT, authorizeRole("admin"), upload.any(), createVideo);

router.route("/details/:id").get(videoDetials);
router.route("/update/:id").put(verifyJWT, authorizeRole("admin"),  upload.fields([{ name: "thumbnail", maxCount: 1 }, { name: "video", maxCount: 1 }, { name: "trailer", maxCount: 1 }]), updateVideo);
